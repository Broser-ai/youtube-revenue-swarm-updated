...
import { CheckCircle, Share2, Award, Sparkles, X, Copy } from 'lucide-react';
import { UserProfile } from '../types';
import { useLanguage } from '../lib/i18n';
import { triggerHaptic, HapticPattern } from '../lib/haptics';

// Import subcomponents directly from the wallet folder
import MembershipStatus from './wallet/MembershipStatus';
import ImpactMetrics from './wallet/ImpactMetrics';
import WeeklyHabitChart from './wallet/WeeklyHabitChart';
import MaterialBreakdown from './wallet/MaterialBreakdown';
import CommunityLeaderboard from './wallet/CommunityLeaderboard';
  ...
  return (
    <div className="w-full max-w-lg mx-auto px-4 pt-4 pb-12 flex flex-col gap-6 select-none animate-in fade-in duration-200 text-left">
      ...
      <div className="flex items-center gap-3 bg-white p-5 rounded-3xl border border-primary/5 shadow-xs">
        <div className="w-10 h-10 bg-[#C8F24A]/20 rounded-xl flex items-center justify-center shrink-0">
          <Award className="w-6 h-6 text-primary" />
        </div>
        <div>
          <h3 className="text-sm font-black uppercase text-primary leading-none">
            {language === 'da' ? 'Miljø & Priser' : 'Impact & Rewards'}
          </h3>
          <p className="text-[10px] text-primary/55 font-bold mt-1">
            {language === 'da' ? 'Følg din grønne stime og se dine klimabesparelser' : 'Track your green accomplishments and carbon offset milestones'}
          </p>
        </div>
      </div>

      {/* 1. Membership Status Block (level, points, badges) */}
      <MembershipStatus user={user} language={language} />

      {/* 2. Personal Green Impact Metrics Dashboard (CO2 etc) */}
      <ImpactMetrics 
        user={user} 
        language={language} 
      ...
      {/* 3. Weekly Habits Bar Chart */}
      <WeeklyHabitChart 
        user={user} 
        language={language} 
      />

      {/* 4. Material Breakdown Donut Chart */}
      <MaterialBreakdown 
        user={user} 
        language={language} 
      />

      {/* 5. Friends & Neighbors Community Leaderboard */}
